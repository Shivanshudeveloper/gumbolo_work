...
router.post("/addfivegoal", (req, res) => {
  const { goaltitle, goalachive, goalhowachive, goalpicture, endDate, userId } =
    req.body;
...
router.post("/addgoal", (req, res) => {
  const { goal1, goal2, goal3, goal4, goal5, todaydate, userId, endDate } =
    req.body;
  ...
  Wrap_Model.countDocuments({ todaydate, userId })
    .then((count) => {
      if (count === 0) {
        ...
        Wrap_Model.findOneAndUpdate(
          { todaydate, userId },
          { goal1, goal2, goal3, goal4, goal5, todaydate, userId },
          { useFindAndModify: false }
        )
          .then(() => {
            res.status(200).json("Marked Completed");
  ...
  SoulPrints_Model.countDocuments({ todaydate, userId })
    .then((count) => {
      if (count === 0) {
        ...
        SoulPrints_Model.findOneAndUpdate(
          { todaydate, userId },
          { soulprint },
          { useFindAndModify: false }
        )
  ...
  Gratitude_Model.countDocuments({ todaydate, userId })
    .then((count) => {
      if (count === 0) {
        ...
        Gratitude_Model.findOneAndUpdate(
          { todaydate, userId },
...
router.get("/longfivegoalsmarkcomplete/:goalId", (req, res) => {
  ...
  Goalfive_Model.findOneAndUpdate(
    { _id: goalId },
    { completed: true },
    { useFindAndModify: false }
  )
...
router.get("/checkdailygoalscount/:userId/:todaydate", (req, res) => {
  ...
  Goals_Model.find({ todaydate, userId })
    .sort({ date: -1 })
    .then((data) => {
      res.status(200).json(data);
    })
...
router.get("/checkdailygratitudecount/:userId/:todaydate", (req, res) => {
  const { userId, todaydate } = req.params;
  Gratitude_Model.find({ todaydate, userId })
    .sort({ date: -1 })
    .then((data) => {
      res.status(200).json(data);
    })
...
router.get("/checkdailysoulprintcount/:userId/:todaydate", (req, res) => {
  const { userId, todaydate } = req.params;
  SoulPrints_Model.find({ todaydate, userId })
    .sort({ date: -1 })
    .then((data) => {
      res.status(200).json(data);
    })
  ...
  Goals_Model.find({ userId })
    ...
    .then((data) => {
      ...
      for (var i = 0; i < fetchedDates.length; i++) {
        ...
        Goals_Model.findOne({ userId, todaydate })
          .sort({ date: -1 })
          .then((data) => {
            allData.push(data);
            Gratitude_Model.findOne({ userId, todaydate })
              .sort({ date: -1 })
              .then((data) => {
                allData.push(data);
                console.log(allData);
...
router.post("/registerbusiness2", (req, res) => {
  ...
  Business_Model.findOneAndUpdate(
    { _id: businessId },
    {
      city,
      state,
  ...
  Organization_Model.countDocuments({ userId })
    .then((count) => {
      if (count === 0) {
        ...
        Organization_Model.findOneAndUpdate(
          { userId },
...
router.post("/savepromotion2", (req, res) => {
  ...
  Promotion_Model.findOneAndUpdate(
    { _id: promotionId },
...
router.post("/registeruser", (req, res) => {
  const { promotionId, firstname, lastname, email, password, browser } =
    req.body;
...
router.get("/fetchpromotiondetails/:promotionId", (req, res) => {
  ...
  res.setHeader("Content-Type", "application/json");
  Promotion_Model.find({ _id: promotionId })
    .sort({ date: -1 })
    .then((data) => {
      res.status(200).json(data);
    })
...
router.get("/fetchpromotiondetailsviews/:promotionId", (req, res) => {
  const { promotionId } = req.params;
  res.setHeader("Content-Type", "application/json");
  Registrations_Model.find({ promotionId })
    .sort({ date: -1 })
    .then((data) => {
      res.status(200).json(data);
    })
    ...
    .then((count) => {
      if (count === 0) {
        ...
        Users_Model.findOneAndUpdate(
          { email },
          { fullName, phoneno, address, zipcode, email },
          { useFindAndModify: false }
        )
          .then(() => {
            res.status(200).json("Users Update");
...
router.post("/addflower", (req, res) => {
  const { name, company, thc, cbd, category, photoDownloadUrl1, price, size } =
    req.body;
...
router.get("/getflowersfiltersize/:s", (req, res) => {
  let { s } = req.params;
  res.setHeader("Content-Type", "application/json");
  Flower_Model.find({ size: { $elemMatch: { size: s } } })
    .sort({ date: -1 })
    .then((data) => {
      res.status(200).json(data);
    })
...
router.get("/getprerollsfiltersize/:s", (req, res) => {
  let { s } = req.params;
  res.setHeader("Content-Type", "application/json");
  AddPreRolls_Model.find({ size: { $elemMatch: { size: s } } })
    .sort({ date: -1 })
    .then((data) => {
      res.status(200).json(data);
    })
...
router.get("/getprerollsfiltercategory/:c", (req, res) => {
  ...
  if (c === "All") {
    ...
    AddPreRolls_Model.find({ category: c })
      .sort({ date: -1 })
      .then((data) => {
        res.status(200).json(data);
      })
...
router.get("/getvapesfiltersize/:s", (req, res) => {
  let { s } = req.params;
  res.setHeader("Content-Type", "application/json");
  Vapes_Model.find({ size: { $elemMatch: { size: s } } })
    .sort({ date: -1 })
    .then((data) => {
      res.status(200).json(data);
    })
...
router.get("/getvapesfiltersubcategory/:sc", (req, res) => {
  let { sc } = req.params;
  res.setHeader("Content-Type", "application/json");
  Vapes_Model.find({ subcategory: sc })
    .sort({ date: -1 })
    .then((data) => {
      res.status(200).json(data);
    })
...
router.get("/getextractsfiltersize/:s", (req, res) => {
  let { s } = req.params;
  res.setHeader("Content-Type", "application/json");
  Extracts_Model.find({ size: { $elemMatch: { size: s } } })
    .sort({ date: -1 })
    .then((data) => {
      res.status(200).json(data);
    })
...
router.get("/getextractsfiltercategory/:c", (req, res) => {
  ...
  if (c === "All") {
    ...
    Extracts_Model.find({ category: c })
      .sort({ date: -1 })
      .then((data) => {
        res.status(200).json(data);
      })
...
router.get("/getextractsfiltersubcategory/:sc", (req, res) => {
  let { sc } = req.params;
  res.setHeader("Content-Type", "application/json");
  Extracts_Model.find({ subcategory: sc })
    .sort({ date: -1 })
    .then((data) => {
      res.status(200).json(data);
    })
...
router.get("/getediblesfiltersize/:s", (req, res) => {
  let { s } = req.params;
  res.setHeader("Content-Type", "application/json");
  Edibles_Model.find({ size: { $elemMatch: { size: s } } })
    .sort({ date: -1 })
    .then((data) => {
      res.status(200).json(data);
    })
...
router.get("/getediblesfiltersubcategory/:sc", (req, res) => {
  let { sc } = req.params;
  res.setHeader("Content-Type", "application/json");
  Edibles_Model.find({ subcategory: sc })
    .sort({ date: -1 })
    .then((data) => {
      res.status(200).json(data);
    })
...
router.get("/gettropicalsfiltersize/:s", (req, res) => {
  let { s } = req.params;
  res.setHeader("Content-Type", "application/json");
  Tropicals_Model.find({ size: { $elemMatch: { size: s } } })
    .sort({ date: -1 })
    .then((data) => {
      res.status(200).json(data);
    })
...
router.get("/gettropicalsfiltercategory/:c", (req, res) => {
  ...
  if (c === "All") {
    ...
    Tropicals_Model.find({ category: c })
      .sort({ date: -1 })
      .then((data) => {
        res.status(200).json(data);
      })
...
router.get("/gettropicalsfiltersubcategory/:sc", (req, res) => {
  let { sc } = req.params;
  res.setHeader("Content-Type", "application/json");
  Tropicals_Model.find({ subcategory: sc })
    .sort({ date: -1 })
    .then((data) => {
      res.status(200).json(data);
    })
  ...
  Aggrements_Model.countDocuments({ uid })
    .then((count) => {
      if (count === 0) {
        ...
        Aggrements_Model.findOneAndUpdate(
          { uid },
          { aggrements },
          { useFindAndModify: false }
        )
